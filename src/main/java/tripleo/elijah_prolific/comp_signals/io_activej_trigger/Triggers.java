...
public final class Triggers implements ConcurrentJmxBean {
	...
	public synchronized void addTrigger(final Trigger trigger) {
		triggers.add(trigger);
	}

	public synchronized void addTrigger(final Severity severity, final String component, final String name, final Supplier<TriggerResult> triggerFunction) {
		triggers.add(Trigger.of(severity, component, name, triggerFunction));
	}

	private void refresh() {
		final long currentTime = now.currentTimeMillis();
		if (cachedTimestamp + CACHE_TIMEOUT.toMillis() < currentTime) {
			cachedTimestamp = currentTime;

			final Map<Trigger, TriggerResult> newResults = new HashMap<>();
			...
			for (final Trigger trigger : new HashSet<>(difference(cachedResults.keySet(), newResults.keySet()))) {
				cachedResults.remove(trigger);
				suppressedResults.remove(trigger);
			}
			for (final Map.Entry<Trigger, TriggerResult> entry : newResults.entrySet()) {
				TriggerResult newResult = entry.getValue();
				if (!newResult.hasTimestamp()) {
					final TriggerResult oldResult = cachedResults.get(entry.getKey());
			...
			for (final Map.Entry<Trigger, TriggerResult> entry : cachedResults.entrySet()) {
				final Trigger       trigger       = entry.getKey();
				final TriggerResult triggerResult = entry.getValue();

				final TriggerWithResult oldTriggerWithResult = maxSeverityResults.get(trigger);
	...
	private List<TriggerWithResult> getResultsBySeverity(@Nullable final Severity severity) {
		refresh();
	...
	@JmxOperation
	public synchronized void suppressTriggerByName(final String name) {
		suppressBy(trigger -> trigger.getName().equals(name));
	}

	@JmxOperation
	public synchronized void suppressTriggerByComponent(final String component) {
		suppressBy(trigger -> trigger.getComponent().equals(component));
	}

	@JmxOperation
	public synchronized void suppressTriggerBySeverity(final String severity) {
	 ...
	 */
	@JmxOperation
	public synchronized void suppressTriggersBySignature(final String signature) {
		final String[] values = signature.split(":");
		if (values.length != 3) {
			return;
		}
	...
	private void suppressBy(final Predicate<Trigger> condition) {
		refresh();
	...
	public final class Builder extends AbstractBuilder<Builder, Triggers> {
		...
		public Builder withTrigger(final Trigger trigger) {
			checkNotBuilt(this);
			addTrigger(trigger);
			return this;
		}
		...
		public Builder withTrigger(final Severity severity, final String component, final String name, final Supplier<TriggerResult> triggerFunction) {
			checkNotBuilt(this);
